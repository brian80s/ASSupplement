...
//    NSLog(@"or:%.f,%.f,%.f,%.f,%.f",self.size.width,self.size.height,scale,width,height);
    self = [UIImage image:self fitInSize:CGSizeMake(width,height)];
    ...
    if (format == ImageFileFormatPNG) {
        ...
        [UIImageJPEGRepresentation(self, quality) writeToFile:filePath atomically:YES];
    }
    return filePath;
}
